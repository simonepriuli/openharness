...
## Feature map
- Desktop chat workspace: `apps/desktop/src/renderer/src/App.tsx`, timeline/event helpers in `events.ts`, chat components under `components/`.
...
- Pi session management: `apps/desktop/src/main/pi-service.ts`, `packages/pi-rpc/src/client.ts`, upstream RPC mode in `vendor/pi/packages/coding-agent/src/modes/rpc`.
