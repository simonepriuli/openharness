...
import { extractResultPayload, stripJsonBlocks } from "./workflow-run-result.js";
...
\`\`\``,
  ...
  it("falls back to generic summary for custom workflows", () => {
    ...
    assert.equal(payload.summary, "Deployment finished successfully.");
  });

  it("returns generic completion message for empty output", () => {
    const payload = extractResultPayload("   ", "manual");
    assert.equal(payload.kind, "generic");
    if (payload.kind !== "generic") return;
    assert.equal(payload.summary, "Workflow completed.");
  });
});
