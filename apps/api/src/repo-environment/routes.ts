...
import { jsonFromHttpResult, tryRepoEnvironment } from "../result-helpers.js";
import {
...
repoEnvironmentRoutes.get("/:connectionId/resolved", async (c) => {
  ...
  const result = await tryRepoEnvironment(() =>
    resolveRepoEnvironmentVariables(db, org.organizationId, connectionId),
  );
  return jsonFromHttpResult(c, Result.map(result, (vars) => ({ vars })));
});
...
repoEnvironmentRoutes.get("/:connectionId", async (c) => {
  ...
  const connectionId = c.req.param("connectionId");
  const result = await tryRepoEnvironment(() =>
    listRepoEnvironmentVariables(db, org.organizationId, connectionId),
  );
  return jsonFromHttpResult(c, Result.map(result, (variables) => ({ variables })));
});
...
repoEnvironmentRoutes.put("/:connectionId/:key", async (c) => {
  ...
  const result = await tryRepoEnvironment(() =>
    upsertRepoEnvironmentVariable(db, org.organizationId, user.id, connectionId, key, {
      value,
      isSecret: body.isSecret === true,
      description: typeof body.description === "string" ? body.description : null,
    }),
  );
  return jsonFromHttpResult(c, Result.map(result, (variable) => ({ variable })));
});
...
repoEnvironmentRoutes.delete("/:connectionId/:key", async (c) => {
  ...
  const result = await tryRepoEnvironment(() =>
    deleteRepoEnvironmentVariable(db, org.organizationId, connectionId, key),
  );
...
repoEnvironmentInternalRoutes.post("/resolve", async (c) => {
  ...
  const result = await tryRepoEnvironment(() =>
    resolveRepoEnvironmentVariables(db, organizationId, connectionId),
  );
  return jsonFromHttpResult(c, Result.map(result, (vars) => ({ vars })));
});
